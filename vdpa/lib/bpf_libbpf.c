...
#define LOXILB_BPF_PATH "/opt/loxilb/dp/bpf"

static int handle_loxilb_tail_calls(struct bpf_object *obj)
{
  int key, fd, jmp_table_fd;
  struct bpf_program *prog;
  const char *section;
  char pin_path[512];
  
  sprintf(pin_path, "%s/%s", LOXILB_BPF_PATH, "pgm_tbl");
  
  jmp_table_fd = bpf_object__find_map_fd_by_name(obj, "pgm_tbl");
  if (jmp_table_fd < 0) {
    jmp_table_fd = bpf_obj_get(pin_path);
    //fprintf(stderr, "jmp_table_fd %s:%d\n", pin_path, jmp_table_fd);
...
static int handle_legacy_maps(struct bpf_object *obj)
{
  ...
  int pinned_map_fd;
    ...
    ret = snprintf(pathname, PATH_MAX, "%s/%s", LOXILB_BPF_PATH, bpf_map__name(map));
    if (ret < 0) {
      return -EINVAL;
    ...
    if (1) { //strcmp(bpf_map__name(map),"pgm_tbl") != 0) { 
      ret = 0;
      pinned_map_fd = bpf_obj_get(pathname);
      if (pinned_map_fd < 0)
        goto try_create;
...
try_create:
		  ...
		  */
		  map_fd = bpf_map__fd(map);
      fprintf(stdout, "mapfd %d:%d \n", map_fd, iproute2_is_pin_map(map_name, pathname));
...
static int load_bpf_object(struct bpf_cfg_in *cfg)
{
	...
	bpf_object__for_each_program(p, obj) {
      ...
      fprintf(stdout, "AUTOLOAD: sec %s prog %s\n",
           get_bpf_program__section_name(p),  bpf_program__name(p));

			ret = bpf_program__set_autoload(p, true);
			if (ret)
				return -EINVAL;
			//continue;
		}
