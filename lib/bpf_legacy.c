...
#define HAVE_LOXILB 1
#define XDP_LL_PROG_ID (2020)
...
#else
    snprintf(pathname, len, "/opt/loxilb/dp/bpf/%s", name);
#endif
		break;
	default:
		snprintf(pathname, len, "%s/../%s/%s",
...
static int bpf_fetch_maps_end(struct bpf_elf_ctx *ctx)
{
	...
	for (i = 0, buff = (void *)ctx->maps; i < ctx->map_num;
		...
		if (fixup[i].type == BPF_MAP_TYPE_PROG_ARRAY &&
			fixup[i].id == 0)
			fixup[i].id = XDP_LL_PROG_ID;
#endif
	}
...
static void bpf_hash_init(struct bpf_elf_ctx *ctx, const char *db_file)
{
    ...
    printf("%s: subpath %s:%d \n", __FUNCTION__, subpath, pinning);
